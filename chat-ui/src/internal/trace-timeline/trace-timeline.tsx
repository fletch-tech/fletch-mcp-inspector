...
/**
 * Same scope contract as `ReadOnlyTranscript`: the package's tokens, waterfall
 * classes, and glyph rules live under `.mcpjam-chat-ui` in styles.css, so the
 * component must apply that class to its own root (and to anything it portals
 * out of the tree) rather than relying on the consumer to wrap it.
 */
...
export interface TraceTimelineProps {
  ...
  fillContent?: boolean;
  /** Theme scope applied to the `.mcpjam-chat-ui` root, like ReadOnlyTranscript. */
  themeMode?: ThemeMode;
}
...
export function TraceTimeline({
  ...
  const scopeClassName = cn("mcpjam-chat-ui", themeClass(themeMode));
  const mode =
  ...
  if (mode === "none") {
    return (
      <div className={cn(scopeClassName, "text-xs text-muted-foreground")}>
  ...
  if (mode === "estimated") {
    return (
      <div className={cn(scopeClassName, "space-y-3")}>
...
function HoveredRowInspector({
  ...
  rowKey: string;
  // `.mcpjam-chat-ui` scope class — the card is portaled to document.body,
  // outside the timeline root, so it must carry the scope itself.
  ...
  return createPortal(
    <div
      className={cn(scopeClassName, "pointer-events-none")}
      style={wrapperStyle}
    >
      <div
        key={rowKey}
