/**
 * Bridge integration with the REAL swarms group (no groups mock): the third
...
describe("swarms group through useSurfaceAgentBridge", () => {
  beforeEach(() => {
  ...
  it("listSurfaceGroupToolNames knows the swarms group", () => {
    expect(listSurfaceGroupToolNames("swarms")).toEqual(SWARMS_TOOL_NAMES);
  });
  ...
  it("mount registers all three tools surface-scoped; unmount removes them", () => {
    const { unmount } = renderHook(() =>
      useSurfaceAgentBridge({
        surfaceId: "swarms",
        snapshot: () => ({ personaCount: 0 }),
      }),
    );

    const state = useUiToolsRegistry.getState();
    for (const name of SWARMS_TOOL_NAMES) {
    ...
    expect(hasSurfaceSnapshotProvider("swarms")).toBe(true);

    unmount();
    for (const name of SWARMS_TOOL_NAMES) {
      expect(useUiToolsRegistry.getState().resolve(name), name).toBeNull();
    }
    expect(hasSurfaceSnapshotProvider("swarms")).toBe(false);
  });
  ...
  it("waitForUiToolNames resolves once the just-mounted group is live (same-turn discovery)", async () => {
    ...
    // ordering when the same turn navigates to /swarms and then waits for
    // the group's tools.
    const pending = waitForUiToolNames(
      listSurfaceGroupToolNames("swarms"),
      1_500,
    );

    const { unmount } = renderHook(() =>
      useSurfaceAgentBridge({ surfaceId: "swarms" }),
    );
    ...
    unmount();
    await expect(
      waitForUiToolNames(listSurfaceGroupToolNames("swarms"), 50),
    ).resolves.toBe(false);
  });
});
