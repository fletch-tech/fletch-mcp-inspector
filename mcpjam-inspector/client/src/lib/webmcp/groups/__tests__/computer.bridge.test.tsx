/**
 * Bridge integration with the REAL computer group (no groups mock): the fifth
...
describe("computer group through useSurfaceAgentBridge", () => {
  beforeEach(() => {
  ...
  it("listSurfaceGroupToolNames knows the computer group", () => {
    expect(listSurfaceGroupToolNames("computer")).toEqual(COMPUTER_TOOL_NAMES);
  });

  it("mount registers all four tools surface-scoped; unmount removes them", () => {
    ...
    expect(hasSurfaceSnapshotProvider("computer")).toBe(true);

    unmount();
    for (const name of COMPUTER_TOOL_NAMES) {
      expect(useUiToolsRegistry.getState().resolve(name), name).toBeNull();
    }
    expect(hasSurfaceSnapshotProvider("computer")).toBe(false);
  });
  ...
  it("waitForUiToolNames resolves once the just-mounted group is live (same-turn discovery)", async () => {
    ...
    // ordering when the same turn navigates to the Computer screen and then
    // waits for the group's tools.
    const pending = waitForUiToolNames(
      listSurfaceGroupToolNames("computer"),
      1_500,
    );

    const { unmount } = renderHook(() =>
      useSurfaceAgentBridge({ surfaceId: "computer" }),
    );
    ...
    unmount();
    await expect(
      waitForUiToolNames(listSurfaceGroupToolNames("computer"), 50),
    ).resolves.toBe(false);
  });
});
