...
- **POST /stream** – Proxies one LLM step for hosted MCPJam chat (`handleMCPJamFreeChatModel` → `CONVEX_HTTP_URL/stream`) and eval agents (`mode: "stream"` vs `mode: "step"`). Register the route in `http.ts` with `http.route({ path: "/stream", method: "POST", handler: streamHttp })` (see `convex/stream.ts`). Set provider keys on the Convex backend: **`OPENAI_API_KEY`** (`openai/*`), **`ANTHROPIC_API_KEY`** (`anthropic/*`), **`GOOGLE_GENERATIVE_AI_API_KEY`** (`google/*`), **`DEEPSEEK_API_KEY`** (`deepseek/*`). The caller must send **`Authorization: Bearer <same Convex JWT as the browser>`** so `getUserIdentity()` succeeds.
