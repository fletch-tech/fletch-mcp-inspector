...
- **POST /stream** – Proxies one LLM step for hosted MCPJam chat (`handleMCPJamFreeChatModel` → `CONVEX_HTTP_URL/stream`) and eval agents (`mode: "stream"` vs `mode: "step"`). Register the route in `http.ts` with `http.route({ path: "/stream", method: "POST", handler: streamHttp })` (see `convex/stream.ts`). Set **`OPENAI_API_KEY`** on the Convex backend for `openai/*` models; set **`ANTHROPIC_API_KEY`** for `anthropic/*` (eval agents). The caller must send **`Authorization: Bearer <same Convex JWT as the browser>`** so `getUserIdentity()` succeeds.
